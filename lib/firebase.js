...
friendlyPix.Firebase = function () {_createClass(_class, null, [{ key: 'POSTS_PAGE_SIZE', 
   ...
   */_createClass(_class, [{ key: 'cancelAllSubscriptions', value: function cancelAllSubscriptions() 
    ...
    pic, thumb, fileName, text) {var _this5 = this;
      ...
      var picCompleter = new $.Deferred();
      picUploadTask.on('state_changed', null, function (error) {
      ...
      var thumbCompleter = new $.Deferred();
      tumbUploadTask.on('state_changed', null, function (error) {
      ...
      return Promise.all([picCompleter.promise(), thumbCompleter.promise()]).then(function (urls) {
        ...
        _this5.database.ref().update(update).then(function () {return newPostKey;});});}
